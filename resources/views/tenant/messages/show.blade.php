	...
	<div class="card mb-5">
		<div class="card-header border-0 pt-6 d-flex flex-wrap align-items-center justify-content-between gap-3">
			<div>
				...
				<p class="text-muted fs-7 mb-0 mt-1">{{ $thread->lease?->property?->name ?? '—' }}</p>
			</div>
			<a href="{{ route('tenant.messages.index') }}" class="btn btn-sm btn-light">{{ __('All messages') }}</a>
		</div>
		<div class="card-body">
			...
			<form method="post" action="{{ route('tenant.messages.store', $thread) }}" class="border-top pt-6">
				@csrf
				<div class="mb-4">
