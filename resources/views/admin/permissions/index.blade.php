	...
	<div class="card">
		<div class="card-header border-0 pt-6">
			...
			<div class="card-toolbar">
				<a href="{{ route('admin.permissions.create') }}" class="btn btn-primary">{{ __('Add permission') }}</a>
			</div>
		</div>
		<div class="card-body py-4">
			<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_admin_permissions_table">
				<thead>
...
@push('scripts')
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			...
			const table = $('#kt_admin_permissions_table').DataTable({
				processing: true,
				serverSide: true,
				ajax: { url: @json(route('admin.permissions.data')), type: 'GET' },
				pageLength: 10,
			...
			$('#kt_admin_permissions_table').on('click', '.js-delete-row', function () {
				const btn = $(this);
