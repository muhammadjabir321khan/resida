...
@section('content')
	<div class="card">
		<div class="card-header"><h2 class="fw-bold m-0">{{ __('Edit maintenance') }}</h2></div>
		<form method="post" action="{{ route('rental.maintenance-requests.update', $maintenance) }}" class="form">
			@csrf @method('PUT')
			<div class="card-body">
				<div class="row mb-6">
					...
					<div class="col-lg-10">
						<select id="property_id" name="property_id" class="form-select form-select-solid @error('property_id') is-invalid @enderror" required>
							@foreach ($properties as $p)
								<option value="{{ $p->id }}" @selected(old('property_id', $maintenance->property_id) == $p->id)>{{ $p->name }}</option>
							@endforeach
						</select>
					...
					<div class="col-lg-4">
						<select id="rental_tenant_id" name="rental_tenant_id" class="form-select form-select-solid @error('rental_tenant_id') is-invalid @enderror">
							...
							@foreach ($tenants as $t)
								<option value="{{ $t->id }}" @selected(old('rental_tenant_id', $maintenance->rental_tenant_id) == $t->id)>{{ $t->full_name }}</option>
							@endforeach
						</select>
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<select id="category" name="category" class="form-select form-select-solid @error('category') is-invalid @enderror">
							...
							@foreach (['plumbing' => __('Plumbing'), 'electrical' => __('Electrical'), 'hvac' => __('HVAC'), 'appliance' => __('Appliance'), 'structure' => __('Structure'), 'other' => __('Other')] as $val => $label)
								<option value="{{ $val }}" @selected(old('category', $maintenance->category) === $val)>{{ $label }}</option>
							@endforeach
						</select>
					...
					<div class="col-lg-4">
						<input id="technician_name" type="text" name="technician_name" value="{{ old('technician_name', $maintenance->technician_name) }}" class="form-control form-control-solid @error('technician_name') is-invalid @enderror" />
				...
				<div class="row mb-6">
					...
					<div class="col-lg-10">
						<input id="title" type="text" name="title" value="{{ old('title', $maintenance->title) }}" class="form-control form-control-solid @error('title') is-invalid @enderror" required />
				...
				<div class="row mb-6">
					...
					<div class="col-lg-10">
						<textarea id="description" name="description" rows="4" class="form-control form-control-solid @error('description') is-invalid @enderror">{{ old('description', $maintenance->description) }}</textarea>
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<input id="estimated_cost" type="number" step="0.01" min="0" name="estimated_cost" value="{{ old('estimated_cost', $maintenance->estimated_cost) }}" class="form-control form-control-solid @error('estimated_cost') is-invalid @enderror" />
					...
					<div class="col-lg-4">
						<input id="actual_cost" type="number" step="0.01" min="0" name="actual_cost" value="{{ old('actual_cost', $maintenance->actual_cost) }}" class="form-control form-control-solid @error('actual_cost') is-invalid @enderror" />
					...
					<div class="col-lg-4">
						<select id="status" name="status" class="form-select form-select-solid @error('status') is-invalid @enderror" required>
							@foreach (['open' => __('Open'), 'in_progress' => __('In progress'), 'completed' => __('Completed'), 'cancelled' => __('Cancelled')] as $val => $label)
								<option value="{{ $val }}" @selected(old('status', $maintenance->status) === $val)>{{ $label }}</option>
							@endforeach
						</select>
					...
					<div class="col-lg-4">
						<select id="priority" name="priority" class="form-select form-select-solid @error('priority') is-invalid @enderror" required>
							@foreach (['low' => __('Low'), 'medium' => __('Medium'), 'high' => __('High'), 'urgent' => __('Urgent')] as $val => $label)
								<option value="{{ $val }}" @selected(old('priority', $maintenance->priority) === $val)>{{ $label }}</option>
							@endforeach
						</select>
				...
				<div class="row mb-0">
					...
					<div class="col-lg-4">
						<input id="reported_on" type="date" name="reported_on" value="{{ old('reported_on', optional($maintenance->reported_on)->format('Y-m-d')) }}" class="form-control form-control-solid @error('reported_on') is-invalid @enderror" />
					...
					<div class="col-lg-4">
						<input id="completed_at" type="datetime-local" name="completed_at" value="{{ old('completed_at', optional($maintenance->completed_at)->format('Y-m-d\TH:i')) }}" class="form-control form-control-solid @error('completed_at') is-invalid @enderror" />
...
@push('scripts')
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			...
			const selectedUnit = @json(old('unit_id', $maintenance->unit_id));
