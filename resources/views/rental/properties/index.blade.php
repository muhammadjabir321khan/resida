...
@section('content')
	<div class="card">
		<div class="card-header border-0 pt-6">
			<div class="card-title"><h2 class="fw-bold m-0">{{ __('Properties') }}</h2></div>
			<div class="card-toolbar">
				<a href="{{ route('rental.properties.create') }}" class="btn btn-primary">{{ __('Add property') }}</a>
			</div>
		</div>
		<div class="card-body py-4">
			<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_rental_properties_table">
				<thead>
...
@push('scripts')
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			...
			const table = $('#kt_rental_properties_table').DataTable({
				processing: true,
				serverSide: true,
				ajax: { url: @json(route('rental.properties.data')), type: 'GET' },
				pageLength: 10,
				order: [[0, 'desc']],
				columns: [
					{ data: 'id', name: 'id' },
			...
			$('#kt_rental_properties_table').on('click', '.js-delete-row', function () {
				const btn = $(this);
				Swal.fire({
