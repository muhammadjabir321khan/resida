...
@section('content')
	<div class="card">
		<div class="card-header"><h2 class="fw-bold m-0">{{ __('Add unit') }}</h2></div>
		<form method="post" action="{{ route('rental.units.store') }}" class="form">
			@csrf
			<div class="card-body">
				<div class="row mb-6">
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<input id="label" type="text" name="label" value="{{ old('label') }}" placeholder="Apt 2B" class="form-control form-control-solid @error('label') is-invalid @enderror" required />
						@error('label')<div class="invalid-feedback">{{ $message }}</div>@enderror
					</div>
					<label class="col-lg-2 col-form-label fw-semibold fs-6" for="unit_type">{{ __('Unit type') }}</label>
					<div class="col-lg-4">
						<input id="unit_type" type="text" name="unit_type" value="{{ old('unit_type') }}" placeholder="1BR, studio…" class="form-control form-control-solid @error('unit_type') is-invalid @enderror" />
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<input id="bedrooms" type="number" name="bedrooms" value="{{ old('bedrooms') }}" min="0" max="50" class="form-control form-control-solid @error('bedrooms') is-invalid @enderror" />
					...
					<div class="col-lg-4">
						<input id="bathrooms" type="number" step="0.1" name="bathrooms" value="{{ old('bathrooms') }}" min="0" class="form-control form-control-solid @error('bathrooms') is-invalid @enderror" />
						...
						<div class="d-flex flex-wrap gap-4">
							@foreach ($amenityOptions as $am)
								<label class="form-check form-check-custom form-check-solid">
									<input class="form-check-input" type="checkbox" name="amenities[]" value="{{ $am }}" @checked(in_array($am, old('amenities', []), true)) />
				...
				<div class="row mb-6">
					...
					<div class="col-lg-10">
						<textarea id="notes" name="notes" rows="2" class="form-control form-control-solid @error('notes') is-invalid @enderror">{{ old('notes') }}</textarea>
