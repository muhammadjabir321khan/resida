...
@section('content')
	<div class="card">
		<div class="card-header"><h2 class="fw-bold m-0">{{ __('Edit transaction') }}</h2></div>
		<form method="post" action="{{ route('rental.transactions.update', $transaction) }}" class="form">
			@csrf @method('PUT')
			<div class="card-body">
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<select id="direction" name="direction" class="form-select form-select-solid @error('direction') is-invalid @enderror" required>
							<option value="income" @selected(old('direction', $transaction->direction) === 'income')>{{ __('Income') }}</option>
							<option value="expense" @selected(old('direction', $transaction->direction) === 'expense')>{{ __('Expense') }}</option>
						</select>
					...
					<div class="col-lg-4">
						<select id="category" name="category" class="form-select form-select-solid @error('category') is-invalid @enderror" required>
							@foreach (['rent' => __('Rent'), 'deposit' => __('Deposit'), 'utilities' => __('Utilities'), 'repair' => __('Repair'), 'insurance' => __('Insurance'), 'tax' => __('Tax'), 'other' => __('Other')] as $val => $label)
								<option value="{{ $val }}" @selected(old('category', $transaction->category) === $val)>{{ $label }}</option>
							@endforeach
						</select>
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<input id="amount" type="number" step="0.01" min="0.01" name="amount" value="{{ old('amount', $transaction->amount) }}" class="form-control form-control-solid @error('amount') is-invalid @enderror" required />
						@error('amount')<div class="invalid-feedback">{{ $message }}</div>@enderror
					</div>
					<label class="col-lg-2 col-form-label required fw-semibold fs-6" for="transaction_date">{{ __('Date') }}</label>
					<div class="col-lg-4">
						<input id="transaction_date" type="date" name="transaction_date" value="{{ old('transaction_date', optional($transaction->transaction_date)->format('Y-m-d')) }}" class="form-control form-control-solid @error('transaction_date') is-invalid @enderror" required />
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<select id="property_id" name="property_id" class="form-select form-select-solid @error('property_id') is-invalid @enderror">
							...
							@foreach ($properties as $p)
								<option value="{{ $p->id }}" @selected(old('property_id', $transaction->property_id) == $p->id)>{{ $p->name }}</option>
							@endforeach
						</select>
					...
					<div class="col-lg-4">
						<select id="lease_id" name="lease_id" class="form-select form-select-solid @error('lease_id') is-invalid @enderror">
							<option value="">{{ __('— None —') }}</option>
							@foreach ($leases as $l)
								<option value="{{ $l->id }}" @selected(old('lease_id', $transaction->lease_id) == $l->id)>#{{ $l->id }} — {{ $l->property?->name ?? '?' }}</option>
							@endforeach
						</select>
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<select id="unit_id" name="unit_id" class="form-select form-select-solid @error('unit_id') is-invalid @enderror">
							<option value="">{{ __('— None —') }}</option>
							@foreach ($units as $u)
								<option value="{{ $u->id }}" @selected(old('unit_id', $transaction->unit_id) == $u->id)>{{ $u->property?->name ?? '?' }} — {{ $u->label }}</option>
							@endforeach
						</select>
					...
					<div class="col-lg-4">
						<input id="vendor_name" type="text" name="vendor_name" value="{{ old('vendor_name', $transaction->vendor_name) }}" class="form-control form-control-solid @error('vendor_name') is-invalid @enderror" />
				...
				<div class="row mb-6">
					...
					<div class="col-lg-4">
						<input id="reference" type="text" name="reference" value="{{ old('reference', $transaction->reference) }}" class="form-control form-control-solid @error('reference') is-invalid @enderror" />
				...
				<div class="row mb-0">
					...
					<div class="col-lg-10">
						<input id="description" type="text" name="description" value="{{ old('description', $transaction->description) }}" class="form-control form-control-solid @error('description') is-invalid @enderror" />
